...
【脚本格式规范（必须严格遵守）】
1. 代码块必须使用正确的 Markdown 围栏格式：\`\`\`bash 开头，\`\`\` 结尾
2. \`\`\`bash 后必须紧跟换行符，不能有多余空格或字符
3. 代码块内的脚本必须可直接复制执行，无需手动修改
4. heredoc 分隔符使用 SCRIPT_EOF 或 INNER_EOF，避免与其他内容冲突
5. 代码块内禁止出现三个连续反引号
...
2. 按照以下结构组织回复：
   - 🔍 **问题分析**：描述观察到的异常
   - 🧠 **可能原因**：列出可能的原因（按可能性排序）
   - ✅ **解决方案**：给出具体的修复命令（用 \`\`\`bash 代码块包裹，\`\`\`bash 后紧跟换行，结束 \`\`\` 独占一行）
   - ⚠️ **预防建议**：如何避免类似问题
...
1. 生成完整的 Shell 脚本，包含：
   ...
   - 详细的注释
   - 日志输出
2. 脚本用 \`\`\`bash 代码块包裹，\`\`\`bash 后必须紧跟换行，结束 \`\`\` 独占一行
3. 代码块内的脚本必须可直接复制执行，不能有占位符或省略号
4. heredoc 分隔符使用有辨识度的名称（如 SCRIPT_EOF），避免与脚本内容冲突
5. 代码块内禁止出现三个连续反引号
6. 在脚本前后给出：
   - 📋 **功能说明**：脚本做什么
   - 📌 **使用方法**：如何运行和传参
   - ⚠️ **注意事项**：权限要求、依赖等
7. 遵循 Shell 脚本最佳实践（引用变量、避免 eval 等）
8. 对于复杂脚本提供测试建议
9. 回复使用中文`
  },
  {
...
2. 分析性能数据时使用结构化输出：
   ...
   - ⚡ **瓶颈分析**：资源瓶颈在哪里
   - 🛠 **优化建议**：具体的优化命令
3. 命令用 \`\`\`bash 代码块包裹，\`\`\`bash 后紧跟换行，结束 \`\`\` 独占一行，命令必须可直接复制执行
...
- 如果你需要生成供用户执行的脚本或指令，必须用 \`\`\`powershell 或 \`\`\`pwsh 包裹代码块，绝不能用 \`\`\`bash！
- \`\`\`powershell 后必须紧跟换行符，结束 \`\`\` 独占一行，代码块内的命令必须可直接复制执行
